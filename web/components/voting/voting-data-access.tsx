	...
	const vote = useMutation({
		...
		mutationFn: async (candidate: string) => {
			...
			const recentPriorityFees = await connection.getRecentPrioritizationFees({
				lockedWritableAccounts: [new PublicKey('DmsZ6Qn3irp5gopdietf8LwTp1MbJLTLtJmjNAhxqtdy')],
			});
			...
			const vote = await program.methods.vote(candidate, new BN(1)).instruction();
...
export function useVotingProgram() {
	...
	const vote = useMutation({
		mutationKey: ['voting', 'vote', { cluster }],
		mutationFn: (candidate: string) => program.methods.vote(candidate, new BN(1)).rpc(),
		onSuccess: (signature) => {
			transactionToast(signature);
